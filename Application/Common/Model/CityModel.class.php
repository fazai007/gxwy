...
class CityModel extends Model
{
     ...
     * @todo 修改模型信息
     */
    public function editCity($where='',$arr)
    {
     ...
     * @todo isuse设为1 || 真删除
     */
    public function delCity($city_id,$opt = false)
    {
        if (!is_numeric($city_id)) return false;
        if($opt)
        {
            return $this->where('city_id = ' . $city_id)->delete();
        }else{
           return $this->where('city_id = ' . $city_id)->save(array('isuse' => 2)); 
        }
     ...
     */
    public function getCityList($fields = '', $where = '', $orderby = '', $group = '')
    {
     ...
     * @return void
     */
    public function getCityField($where,$field)
    {
     ...
     * @todo 根据传入的$City_list获取更详细的模型列表页数据信息列表
     */
     ...
     * @todo 根据SQL查询字句查询模型信息
     */
    public function getAllCity($fields = '', $where = '', $orderby = '', $group = '')
    {
