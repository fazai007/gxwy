...
class Auth {
    ...
    public function check($name, $user_id, $type = 1, $mode = 'url', $relation = 'or') {
        if (!$this->_config['AUTH_ON'])
            return true;
        $authList = $this->getAuthList($user_id, $type); //获取用户需要验证的所有有效规则列表
        if (is_string($name)) {
        ...
        if ($mode == 'url') {
            $REQUEST = unserialize(serialize($_REQUEST));//strtolower()
        }
     ...
     *     array('user_id'=>'用户id','group_id'=>'用户组id','title'=>'用户组名称','rules'=>'用户组拥有的规则id,多个,号隔开'),
     *     ...)   
     */
    public function getGroups($user_id) {
        ...
        $groups[$user_id] = $user_groups? : array();
        return $groups[$user_id];
    }
    ...
    protected function getAuthList($user_id, $type) {
        ...
        if ($this->_config['AUTH_TYPE'] == 2 && isset($_SESSION['_AUTH_LIST_' . $user_id . $t])) {
        ...
        if ($this->_config['AUTH_TYPE'] == 2) {
            //规则列表结果保存到session
            $_SESSION['_AUTH_LIST_' . $user_id . $t] = $authList;
        }
    ...
    protected function getUserInfo($user_id) {
        ...
        if (!isset($userinfo[$user_id])) {
            $userinfo[$user_id] = M()->where(array('user_id' => $user_id))->table($this->_config['AUTH_USER'])->find();
        }
