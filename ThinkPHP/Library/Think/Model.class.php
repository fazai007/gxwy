...
class Model {
     ...
     * @return ['total','page_size','start','root','total_page','current_page']
     */
    public function pageQuery($sql, $page = 0, $page_size = 0) {
        $page_size = (intval($page_size) == 0) ? C('PAGE_SIZE') : $page_size;
        if ($page_size == 0)
            return [];
        ...
        $pager = [];
        //查询总数
        $total_sql = "select count(1) counts from (" . $sql . ") as a";
        $total = $this->query($total_sql);
        //查询数据		
        $result = $this->query($sql . " limit " . $start . "," . $page_size);
        //计算页码信息
        $pager['total'] = $total[0]['counts'];
        $pager['page_size'] = $page_size;
        $pager['start'] = $start;
        $pager['root'] = $result;
        $pager['total_page'] = ($pager['total'] % $page_size == 0) ? ($pager['total'] / $page_size) : (intval($pager['total'] / $page_size) + 1);
